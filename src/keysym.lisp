...
(defcstruct SDL-key-sym
  (scancode :unsigned-char)
  (sym SDL-Key)
  (mod :int) ;SDL-Mod
  (unicode :unsigned-short))

(defconstant SDL-ALL-HOTKEYS #xFFFFFFFF)

(defcfun ("SDL_EnableUNICODE" SDL-Enable-UNICODE) :int
  (enable :int))
...
(defcfun ("SDL_EnableKeyRepeat" SDL-Enable-Key-Repeat) :int
  (delay :int)
  (interval :int))

(defcfun ("SDL_GetKeyRepeat" SDL-Get-Key-Repeat) :void
  (delay :pointer)
  (interval :pointer))

(defcfun ("SDL_GetKeyState" SDL-Get-Key-State) :pointer
  (numkeys :pointer))

(defcfun ("SDL_GetModState" SDL-Get-Mod-State) :int)

(defcfun ("SDL_SetModState" SDL-Set-Mod-State) :void
  (modstate :int)) ;Sdl-Mod

(defcfun ("SDL_GetKeyName" SDL-Get-Key-Name) :string
  (key Sdl-Key))

(defconstant SDL-KEY-MOD-CTRL	(logior  (foreign-enum-value 'Sdl-Mod :SDL-KEY-MOD-LCTRL)
					 (foreign-enum-value 'Sdl-Mod :SDL-KEY-MOD-RCTRL)))
(defconstant SDL-KEY-MOD-SHIFT	(logior  (foreign-enum-value 'Sdl-Mod :SDL-KEY-MOD-LSHIFT)
					 (foreign-enum-value 'Sdl-Mod :SDL-KEY-MOD-RSHIFT)))
(defconstant SDL-KEY-MOD-ALT	(logior  (foreign-enum-value 'Sdl-Mod :SDL-KEY-MOD-LALT)
					 (foreign-enum-value 'Sdl-Mod :SDL-KEY-MOD-RALT)))
(defconstant SDL-KEY-MOD-META	(logior  (foreign-enum-value 'Sdl-Mod :SDL-KEY-MOD-LMETA)
					 (foreign-enum-value 'Sdl-Mod :SDL-KEY-MOD-RMETA)))
